...
namespace ts {
    ...
    export function transformES2015(context: TransformationContext) {
        ...
        const typeChecker = compilerOptions.accessorOptimization ? context.getEmitHost().getTypeChecker!() : undefined;
        ...
        function transformJumpTarget(statements: Statement[], receiver: LeftHandSideExpression, member: MethodDeclaration): void {
            ...
            statements.push(factory.createExpressionStatement(
                factory.createAssignment(memberName, <Identifier>member.name),
            ));
        ...
        function transformAccessorsToExpression(receiver: LeftHandSideExpression, { firstAccessor, getAccessor, setAccessor }: AllAccessorDeclarations, container: Node, startsOnNewLine: boolean): Expression {
            ...
            if (getAccessor) {
                ...
                const getter = factory.createPropertyAssignment("get", getterExpression);
            ...
            if (setAccessor) {
                ...
                const setter = factory.createPropertyAssignment("set", setterExpression);
        ...
        function createAccessorExpression(accessor: AccessorDeclaration, firstAccessor: AccessorDeclaration, receiver: LeftHandSideExpression, container: Node): Expression {
            ...
            const method = compilerOptions.accessorOptimization ? getJumpTargetOfAccessor(accessor) : undefined;
            if (method) {
                const methodName = <Identifier>factory.cloneNode(method.name);
                ...
                if (firstAccessor.pos > method.pos) { // the target method has been already emitted.
                    const target = factory.cloneNode(receiver);
                    setEmitFlags(target, EmitFlags.NoComments | EmitFlags.NoTrailingSourceMap);
                    setSourceMapRange(target, firstAccessor.name);
                    expression = factory.createPropertyAccessExpression(target, methodName);
                }
                else {
        ...
        function getJumpTargetOfAccessor(accessor: AccessorDeclaration) {
            ...
            const expression = (<ReturnStatement>statement).expression;
            ...
            const propertyExpression = <PropertyAccessExpression>callExpression.expression;
            if (propertyExpression.expression.kind !== SyntaxKind.ThisKeyword) {
                return;
            }
            const symbol = typeChecker && typeChecker.getSymbolAtLocation(propertyExpression.name);
            if (!symbol) {
                return;
            }
